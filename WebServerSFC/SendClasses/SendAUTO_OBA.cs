...
namespace SentinelaRoku.SendClasses
{
    class SendAUTO_OBA : IDisposable
    {
        ...
        /*************************************************************************************************************************/
        /*--- Construtor ---*/
        public SendAUTO_OBA(string id, string host, string group, DataTable stations, DataTable errorCodes)
        {
            operatorID = id;
            ...
            try
            {
                ...
                else if (Receive.Contains("2>>")) //Logout no Webservice
                {
                    ...
                    if (ResultTest == "PASS")
                    {
                        ...
                        //    alertMessage.InsertMessage(resultLogout.ErrorMessage);
                        //    alertMessage.Show();
        ...
        public List<object> SendMessageToTest(string sendMessage)
        {
            ...
            try
            {
                ...
                System.IO.File.AppendAllLines($@"{ConfigurationManager.AppSettings["LogFileOBA"]}\{DateTime.Now.ToString("yyyyMMdd")}-Log.txt", mensagem, System.Text.Encoding.ASCII);
