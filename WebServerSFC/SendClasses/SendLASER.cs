...
namespace SentinelaRoku.SendClasses
{
    class SendLASER : IDisposable
    {
        ...
        /*************************************************************************************************************************/
        /*--- Construtor ---*/
        public SendLASER(string id, string host, string group, DataTable stations, DataTable errorCodes)
        {
            operatorID = id;
        ...
        public List<object> messageAnalysis(string Receive)
        {
            ...
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"SendLASER.cs Flag-1: {ex.Message}");
                }
                MessageBox.Show($"SendLASER.cs Flag-1: {ex.Message}", "Alert", MessageBoxButton.OK, MessageBoxImage.Warning);
                throw;
            }
            /*-----------------------------------------------------------------------------------------------------------------------*/
        ...
        public List<object> SendMessageToTest(string sendMessage)
        {
            ...
            try
            {
                ...
                System.IO.File.AppendAllLines($@"{ConfigurationManager.AppSettings["LogFileLASER"]}\{DateTime.Now.ToString("yyyyMMdd")}-Log.txt", mensagem, System.Text.Encoding.ASCII);
            ...
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"SendLASER.cs Flag-2: {ex.Message}");
                }

                MessageBox.Show($"SendLASER.cs Flag-2: {ex.Message}", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;
            }
