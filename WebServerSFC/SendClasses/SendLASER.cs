...
namespace SentinelaRoku.SendClasses
{
    class SendLASER : IDisposable
    {
        ...
        /*************************************************************************************************************************/
        /*--- Construtor ---*/
        public SendLASER(string id, string host, string group)
        {
            operatorID = id;
        ...
        private void LoadDataTableErrorCode()
        {
            ...
            using (OleDbConnection conn = this.returnConnection(fileName))
            {
                try
                {
                    conn.Open();
                    // retrieve the data using data adapter
                    OleDbDataAdapter sheetAdapter = new OleDbDataAdapter($"select * from [LASER$]", conn);
        ...
        public List<object> SendMessageToTest(string sendMessage)
        {
            ...
            try
            {
                ...
                File.AppendAllText($@"{ConfigurationManager.AppSettings["LogFileLASER"]}\{DateTime.Now.ToString("yyyyMMdd")}-Log.txt", string.Join(Environment.NewLine, sendMessage));
            ...
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"SendLASER.cs Flag-OnFileChanged: {ex.Message}");
                }

                MessageBox.Show($"SendLASER.cs Flag-: {ex.Message}", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;
            }
