...
namespace SentinelaRoku.SendClasses
{
    class SendRC : IDisposable
    {
        ...
        /*************************************************************************************************************************/
        /*--- Construtor ---*/
        public SendRC(string id, string host, string group, DataTable stations, DataTable errorCodes)
        {
            operatorID = id;
        ...
        public List<object> messageAnalysis(string Receive)
        {
            ...
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"SendRC.cs Flag-1: {ex.Message}");
                }
                MessageBox.Show($"SendRC.cs Flag-1: {ex.Message}", "Alert", MessageBoxButton.OK, MessageBoxImage.Warning);
                throw;
            }
            /*-----------------------------------------------------------------------------------------------------------------------*/
        ...
        public List<object> SendMessageToTest(string sendMessage)
        {
            ...
            try
            {
                ...
                System.IO.File.AppendAllLines($@"{ConfigurationManager.AppSettings["LogFileRC"]}\{DateTime.Now.ToString("yyyyMMdd")}-Log.txt", mensagem, System.Text.Encoding.ASCII);

                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"File sent for testing: {$@"{ConfigurationManager.AppSettings["LogFileRC"]}\{DateTime.Now.ToString("yyyyMMdd")}-Log.txt"}");
                }
            }
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"SendRC.cs Flag-2: {ex.Message}");
                }

                MessageBox.Show($"SendRC.cs Flag-2: {ex.Message}", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;
            }
