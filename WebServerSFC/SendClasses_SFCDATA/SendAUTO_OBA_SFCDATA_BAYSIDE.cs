...
{
    class SendAUTO_OBA_SFCDATA_BAYSIDE: IDisposable
    {
        ...
        /*************************************************************************************************************************/
        /*--- Construtor ---*/
        public SendAUTO_OBA_SFCDATA_BAYSIDE(string id, string host, string group, DataTable stations, DataTable errorCodes)
        {
            operatorID = id;
        ...
        public RegMessageAnalysis messageAnalysis(string Receive)
        {
            ...
            try
            {
                        ...
                        if (Hostname == hostName)
                        {
                            ...
                            if (resultCheckStatus.StatusCode == "0")
                            {
                                ...
                                if (resultGetData.StatusCode == "0") //check OK
                                {
                                    ...
                                    regMessageAnalysis.testAnswer = $"1>>SERIALNO={SN},FW={FW},CSN={CSN},CESN={CESN},PNNAME={PN}#OK,UNIT STATUS IS VALID";
                                }
                                else if (resultGetData.StatusCode == "1")   //check not OK
                                {
                                    regMessageAnalysis.resultTest = false;

                                    regMessageAnalysis.testAnswer = $"1>>SERIALNO={SN},FW={FW},CSN={CSN},CESN={CESN},PNNAME={PN}#{GetDataErrorMessage}";
                                }
                            }
                            else
                            {
                                ...
                                regMessageAnalysis.testAnswer = $"1>>SERIALNO={SN},FW={FW},CSN={CSN},CESN={CESN},PNNAME={PN}#{statusMessage}";
                ...
                else if (Receive.Contains("2>>")) //Logout no Webservice
                {
                    ...
                    string SN = componetMessage[0];
                    string FW = componetMessage[1].Split('=')[1];
            ...
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")} --> SendAUTO_OBA_SFCDATA.cs Flag-1: {ex.Message}");
                }

                MessageBox.Show($"SendAUTO_OBA_SFCDATA.cs Flag-1: {ex.Message}", "SentinelaRoku ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;
            }
            /*-----------------------------------------------------------------------------------------------------------------------*/
        ...
        public RegMessageAnalysis SendMessageToTest(string sendMessage, string testStage)
        {
            ...
            catch (Exception ex)
            {
                using (var writeLog = new WriteLog())
                {
                    writeLog.WriteLogFile($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")} --> SendAUTO_OBA_SFCDATA.cs Flag-2: {ex.Message}");
                }

                MessageBox.Show($"SendAUTO_OBA_SFCDATA_BAYSIDE.cs Flag-2: {ex.Message}", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;
            }
